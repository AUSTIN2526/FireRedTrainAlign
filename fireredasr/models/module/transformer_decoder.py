...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
                ...
                src_mask: Tensor) -> Tensor:
        """
        训练时的前向计算（teacher forcing）：
          - ys:         (batch, T_tgt)   —— 已在最前面加上 sos_id，padding 用 pad_id
          - encoder_outputs: (batch, T_enc, d_model)
          - src_mask:   (batch, 1, T_enc) 或 (batch, T_enc) 的 mask，0 表示 padding
        返回：
          - logits:     (batch, T_tgt, odim)
        """
        # 1. 构造 target 上三角掩码（屏蔽未来位置）+ padding 掩码
        tgt_mask = self.ignored_target_position_is_0(ys, self.pad_id)  # (batch, 1, T_tgt)

        # 2. 词嵌入 + 位置编码
        #    emb: (batch, T_tgt, d_model)
        dec_output = self.dropout(
            self.tgt_word_emb(ys) * self.scale +
            self.positional_encoding(ys)
        )
        ...
        # 4. LayerNorm + 投影到词表
        dec_output = self.layer_norm_out(dec_output)            # (batch, T_tgt, d_model)
        logits = self.tgt_word_prj(dec_output)                  # (batch, T_tgt, odim)
        return logits
